...
class AgentFlowTests(unittest.TestCase):
    ...
    def test_final_critic_gate_and_no_write_authority(self) -> None:
        ...
        with tmp:
            ...
            self.assertTrue(any("write authority" in error for error in agent_flow.validate_task_state(target, "001_toy")))
            agent_flow.write_json(root / "FINAL_CRITIC_AUDIT.json", {"decision": "CRITIC_FINAL_PASS", "touched_paths": []})
    ...
    def test_terminal_notifier_brief_only_for_terminal_user_states(self) -> None:
        ...
        with tmp:
            ...
            current["state"] = "AWAIT_HUMAN_DECISION"
            agent_flow.write_json(root / "CURRENT.json", current)
            brief = agent_flow.terminal_notification_brief(target, "001_toy")
    ...
    def test_toy_a_control_plane_path(self) -> None:
        ...
        with tmp:
            ...
            root = agent_flow.task_root(target, "001_toy")
            agent_flow.write_json(root / "FINAL_CRITIC_AUDIT.json", {"decision": "CRITIC_FINAL_PASS", "review_target_id": snapshot["review_target_id"], "touched_paths": []})
            current = agent_flow.load_json(root / "CURRENT.json")
            current["state"] = "AWAIT_HUMAN_DECISION"
            agent_flow.write_json(root / "CURRENT.json", current)
            self.assertEqual(agent_flow.validate_task_state(target, "001_toy"), [])
