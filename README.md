...
当前集成版本：`0.9.1`（复核模式首次创建的正常入口已完成候选实现；这不是 GitHub 正式发布或标签声明）。
...
├── 复核模式              GPT 先规划，Codex 执行，再由 GPT 独立复核
├── 控制模式              高风险任务的严格闭环
├── 持久运行              长期目标的 tmux 持久执行能力
├── 通知                  任务结束或运行层进展后发通知
├── Overleaf 桥接         只把论文目录同步到 Overleaf
├── 视觉复核              对图片、PPT 截图等做独立视觉检查
└── 文本复核 / 转换
...
持久运行是项目层可选能力，不是第四套工作流。轻量交接 / 复核模式 / 控制模式三档工作流保持不变；持久运行只解决长期任务在 Codex / SSH 断开后如何继续运行、如何恢复、以及如何避免临时发明后台机制的问题。
...
GPT 写长期目标/任务时要先判断执行生命周期。`run overnight`、`unattended`、`run until morning`、`leave it running`、`survive disconnect`，或者中文里“跑到明早”“断线后继续”“恢复持久目标”这类要求，都意味着需要持久运行合同；但这不代表必须把轻量交接任务改成复核模式 / 控制模式。如果仓库没有安装持久运行，也没有用户选择的等价项目原生合同，不要把任务写成普通实时 Codex 会话。
